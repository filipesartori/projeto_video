...
def index():
    if request.method == 'POST':
        ...
        os.mkdir('./video')
        os.mkdir(f'./{nome}')
        roteiro2 = roteiro('Crie um texto que será falado de NO MAXIMO 500 caracteres, para o tiktok, com curiosidades, SEM HASHTAGS, sobre o tema:', nome)
        voz(roteiro2)
        downloader.download(nome, limit=15,  output_dir='/workspaces/projeto_video/projeto_video/app/video', adult_filter_off=True, force_replace=False, timeout=60, verbose=True)
        imagem(nome)
        make_video(nome)
